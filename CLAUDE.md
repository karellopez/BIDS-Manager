...
`/Users/karelo/PycharmProjects/superbidsmanager/BIDS-Manager/` is the **`bids-manager`** package on PyPI. A schema-driven BIDS converter, curator, and editor. v1.0.0 is a complete re-imagination of the 0.x line; the old HeuDiConv / dcm2bids two-engine GUI is gone. The PyPI distribution name is `bids-manager`, the Python import name is `bidsmgr` (same pattern as `pip install scikit-learn` then `import sklearn`).

| Path | Purpose |
|---|---|
| `bidsmgr/` | The importable Python package (16 subpackages). |
| `tests/` | ~454 unit + ~394 GUI + ~49 real-data tests (real-data gated on env vars). |
| `docs/` | Design history + PR write-up: `architecture.md`, `super_plan.md`, `improvement_plan.md`, `gui_mockups.html`, `inspector_proto/`, `workflow.svg`, `screenshots/`, and **`PULL_REQUEST.md`** (the canonical long-form v1.0.0 cutover narrative for upstream reviewers). |
| `miscellaneous/` | README GIFs (`hero.gif`, `see.gif`, `fix.gif`, `inspect.gif`, `validate.gif`) plus `images/Logo_negative_square.png` (preserved from 0.x, currently unused). |
| `external/` | Vendored Python embed used by the Windows installer. |
| `Installers/` | Packaged Windows installer (`Installers.zip`). Targets the 0.x console scripts; rebuilding to wire the new `bidsmgr-*` scripts is on the post-v1.0.0 todo list. |

The historical v0.2.5 codebase (`bids_manager/` Python package, HeuDiConv + dcm2bids drivers, 9.2k-LOC single-file GUI) is gone from `main` but lives in git history through commit `6bee846` and earlier.

## Architecture (read this before adding features)

The single design bet: **the BIDS schema is the engine**, not a check at the end. Every layer reads from the same `bidsschematools` schema. See `docs/architecture.md` §0 to §3 for the rationale and §12 for the module layout. `docs/PULL_REQUEST.md` §3 to §16 covers the same ground at the implementation level.
...
5. Pure-data types only (Pydantic v2 / dataclass; no I/O methods).
6. Functions over classes where possible.

## Current state (v1.0.0)

**Latest commits on `karellopez/BIDS-Manager` `main`:**

```
0d3ef78 docs: extensive PR write-up for the v1.0.0 cutover
f1165e1 docs: rewrite README for end users, add workflow diagram and GIFs
14cc042 v1.0.0: complete re-imagination - schema-driven engine + Converter/Editor GUI
6bee846 Add negative logo for brain hack   <- last v0.2.5-line commit before the cutover
```

`14cc042` is the squashed cutover commit. `f1165e1` adds the user-facing README, the workflow SVG, and the five README GIFs in `miscellaneous/`. `0d3ef78` adds the full PR write-up at `docs/PULL_REQUEST.md`.
...
- **`project/`** event-sourced `.bidsmgr/` bundles. Events: `ProjectCreated`, `ScanImported`, `UserSet{Cell,Entity}`, `UserToggleInclude`, `TodoAcknowledged`, `StageCompleted`. Wired through the Converter GUI; not yet wired through CLI verbs.
...
### GUI (`bidsmgr.gui.*`, launched via the `bidsmgr` console script)

- **MainWindow shell** top header (clickable brand to `AboutDialog`; `Converter | Editor` pill switcher; theme toggle) + `QStackedWidget` + status bar. Active view persists via `AppSettings`.
...
- `theme`: "dark" | "light"
- `active_view`: "converter" | "editor"
- `editor_bids_root`: last opened BIDS root in Editor
- `editor_sidecar_view`: "bids" | "tree"
- `editor_strict_validate`: bool. When True, ReportWorker runs with strict=True
- `nifti_crosshair_color`: hex string (default `#4FC3F7`)
- `nifti_crosshair_thickness`: int 1..5 (default 1)
- (plus the Converter's Scan and Convert defaults under `scan/*` and `convert/*` namespaces)

### Tests

~454 unit + ~394 GUI + ~49 real-data. Real-data gated on `BIDS_MANAGER_REAL_{MRI,EEG,MEG}_DATA=1`. GUI tests under `QT_QPA_PLATFORM=offscreen` via `pytest-qt`. Known intermittent: pytest-qt teardown can segfault on full-suite run (stale paint after a model goes out of scope); individual test files run clean.
...
Plus the GUI entry: `bidsmgr [--theme dark|light] [--project PATH.bidsmgr]`.
...
2. **3-D viewer + GIFTI/FreeSurfer surface viewer** port `Volume3DDialog` (~1500 LOC pyqtgraph/scikit-image) + `Surface3DDialog` from the v0.2.5 codebase (preserved in git history). Would add `scikit-image` back to deps.
...
6. **Lower priority**: `.cnt` Neuroscan format hint; wire `--project` through CLI verbs; rebuild the Windows installer in `Installers/` against the new `bidsmgr-*` scripts; track down the pytest-qt teardown segfault.

## User preferences

- **No `Co-Authored-By: Claude` trailer** on commits.
- **No em dashes** in user-facing or documentation text (use periods, colons, or hyphens). Verbatim quotes of historical commit messages may keep theirs.
- **No emojis** in the user-facing README. Feature headings are plain numbered.
