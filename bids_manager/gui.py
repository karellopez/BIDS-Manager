...
class BIDSManager(QMainWindow):
    ...
    def loadSequenceDictionary(self) -> None:
        ...
        if not entries:
            ...
            for mod, rule in dicom_inventory.BIDS_HINTS.items():
                entries[mod].extend(rule.patterns)
    ...
    def applySequenceDictionary(self) -> None:
        ...
        dicom_inventory.override_hint_patterns({m: tuple(pats) for m, pats in patterns.items()})
        if self.mapping_table.rowCount() > 0:
            for i in range(self.mapping_table.rowCount()):
