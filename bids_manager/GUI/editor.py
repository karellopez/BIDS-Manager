"""BIDS dataset editor widgets and helpers.

All GUI components related to browsing and editing BIDS datasets live here,
separate from the converter so each concern can evolve independently.
"""
...
class RemapDialog(QDialog):
    ...
    def apply(self):
        ...
        if rename_map:
            try:
                from bids_manager.scans_utils import update_scans_with_map
...
class EditorMixin:
    """Editor tab and metadata viewer integration.

    This mixin retains the editor tab setup alongside helpers and dialogs
    previously bundled in the monolithic GUI file.
    """
    ...
    def refreshScansTsv(self):
        ...
        if not self.bids_root:
            ...
            return
        try:
            from bids_manager.post_conv_renamer import update_scans_tsv
