"""Converter tab widgets and logic for BIDS Manager.

This module isolates the converter tab UI and the associated DICOM→BIDS
pipeline helpers so they are no longer intertwined with the editor code.
"""
...
try:  # prefer relative import but fall back to direct when running as a script
    ...
    from bids_manager.run_heudiconv_from_heuristic import is_dicom_file  # type: ignore
try:
...
class ConverterMixin:
    """Converter tab logic and conversion pipeline orchestration.

    This mixin houses the converter tab UI creation alongside all supporting
    behaviours originally embedded in the monolithic GUI module. It relies on
    the host QMainWindow for status management, theming and shared state.
    """
